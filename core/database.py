...
class DatabaseManager:
    ...
    def __init__(self, config=None):
        dsn = os.getenv("MARKET_DB_URL", "").strip()
        if not dsn:
            # Fallback to config if available, or error
            if config and hasattr(config, 'DB_URL') and config.DB_URL:
                dsn = config.DB_URL
            else:
                raise RuntimeError("MARKET_DB_URL is not set in .env")
        self.dsn = dsn
