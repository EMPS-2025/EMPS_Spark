...
async def handle_message(msg: cl.Message):
    ...
    try:
        ...
        deriv_data, deriv_actual_date = db.fetch_derivative_data(specs[0].start_date)
                ...
                # Build a human-readable note for the derivatives panel
        deriv_note = None
        if deriv_actual_date:
            try:
                req_date = specs[0].start_date
                if isinstance(deriv_actual_date, date):
                    if deriv_actual_date == req_date:
                        deriv_note = f"Futures close as of {deriv_actual_date.strftime('%d %b %Y')}."
                    else:
                        deriv_note = (
                            f"Last available close: {deriv_actual_date.strftime('%d %b %Y')} "
                            f"(no futures settlement on requested date)."
                        )
                else:
                    deriv_note = f"Futures close as of {deriv_actual_date}."
            except Exception:
                # Fail silently – dashboard will still render without the note
                deriv_note = None
        ...
        # Build date label
                # Build date label (dd/mm/yyyy for UI)
        def fmt_dmy(d: date) -> str:
            return d.strftime("%d/%m/%Y")

        if len(specs) > 1:
            # keep the "N Periods" behavior for complex queries
            date_label = f"{len(specs)} Periods"
            date_range_label = f"{fmt_dmy(specs[0].start_date)} to {fmt_dmy(specs[-1].end_date)}"
        else:
            start = specs[0].start_date
            end = specs[0].end_date
            if start == end:
                # single day → dd/mm/yyyy
                date_label = fmt_dmy(start)
                date_range_label = start.strftime("%d %b %Y")
            else:
                # range → dd/mm/yyyy to dd/mm/yyyy
                date_label = f"{fmt_dmy(start)} to {fmt_dmy(end)}"
                date_range_label = f"{start.strftime('%d %b')} to {end.strftime('%d %b %Y')}"

        # Build response sections
        response_text = response_builder.build_dashboard(
            primary_market=primary_market,
            date_label=date_label + exclusion_label,
            time_label=selection_details['time_label'],
            market_data=all_market_data,
        ...
        final_elements = [primary_chart] if primary_chart else []
